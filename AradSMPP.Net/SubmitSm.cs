...
public class SubmitSm : Header, IPacket, IPduDetails
{
    ...
    {
        SubmitSm? submitSm = new(defaultEncoding);
        ...
        try
        {
            ...
            {
                submitSm.UserDataBuffer = new(defaultEncoding, buf.ExtractByteArray(ref offset, submitSm.ShortMessageLength));
            }

            while (offset - startOffset < submitSm.Length)
            {
            ...
            if (submitSm.ShortMessageLength == 0)
            {
                ...
                {
                    Tlv tlvPayload = submitSm.Optional[OptionalTags.MessagePayload];
                    if (tlvPayload != null)
                    {
                        submitSm.UserDataBuffer = new(defaultEncoding, tlvPayload.Value);
                    }
                    
                    Tlv tlvGlobalSeq = submitSm.Optional[OptionalTags.SarMsgRefNum];
                    if (tlvGlobalSeq != null)
                    {
                        submitSm.MessageReferenceNumber = SmppBuffer.BytesToShort(tlvGlobalSeq.Value, 0);
                    }
                    
                    Tlv tlvPocketsNumber = submitSm.Optional[OptionalTags.SarTotalSegments];
                    if (tlvPocketsNumber != null)
                    {
                        submitSm.TotalSegments = tlvPocketsNumber.Value[0];
                    }
                    
                    Tlv tlvLocalSeq = submitSm.Optional[OptionalTags.SarSegmentSeqnum];
                    if (tlvLocalSeq != null)
                    {
                        submitSm.SeqmentNumber = tlvLocalSeq.Value[0];
                    }
                }
            }
