...
public class SmppBuffer
{
    ...
    public void AddTimeString(string? value, int length)
    {
        ...
        StringBuilder valueBuffer = new(value);
    ...
    {
        Header header = new(DefaultEncoding);
    ...
    public void ExtractHeader(Header? header, ref int offset)
    {
        ...
        header.PduData = new(header.DefaultEncoding, ListBuffer.GetRange(startingOffset, (int) header.Length));
    }
    ...
    {
        List<UnsuccessDestinationAddress> unsuccessDestinationAddresses = [];
    ...
    {
        PduPropertyDetail detail = new(offset, PduDataTypes.UInt, propertyName) { DataBlock = ListBuffer.GetRange(offset, 4).ToArray() };
    ...
    {
        PduPropertyDetail detail = new(offset, PduDataTypes.UShort, propertyName) { DataBlock = ListBuffer.GetRange(offset, 2).ToArray() };
    ...
    {
        PduPropertyDetail detail = new(offset, PduDataTypes.Byte, propertyName) { DataBlock = new byte[1] };
    ...
    {
        PduPropertyDetail detail = new(offset, PduDataTypes.ByteArray, propertyName) { DataBlock = ListBuffer.GetRange(offset, length).ToArray() };
    ...
    {
        PduPropertyDetail detail = new(offset, PduDataTypes.CString, propertyName);
    ...
    {
        PduPropertyDetail detail = new(offset, PduDataTypes.EncodedString, propertyName);
    ...
    public List<PduPropertyDetail> ExtractHeaderDetails(ref int offset)
    {
        ...
        [
            ExtractInt("Length", ref offset),
            ExtractInt("CommandSet", ref offset),
            ExtractInt("CommandStatus", ref offset),
            ExtractInt("Sequence", ref offset)
        ];
    ...
    public void ExtractUserData(List<PduPropertyDetail> details, bool udhi, int offset)
    {
        ...
        if (Length - offset > 0)
        {
            detail = new(offset, PduDataTypes.ByteArray, "UserData_Payload") { DataBlock = ListBuffer.GetRange(offset, Length - offset).ToArray() };
            details.Add(detail);
        }
    }
    ...
    {
        List<byte> messageBuffer = [..messageBytes];
