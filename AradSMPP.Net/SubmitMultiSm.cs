...
public class SubmitMultiSm : Header, IPacket, IPduDetails
{
    ...
    {
        return new(defaultEncoding, serviceType, srcTon, srcNpi, srcAddr, destList);
    }
    ...
    {
        SubmitMultiSm? submitMulti = new(defaultEncoding);
        ...
        try
        {
            ...
            {
                submitMulti.UserDataBuffer = new(defaultEncoding, buf.ExtractByteArray(ref offset, submitMulti.ShortMessageLength));
            }
            
            while (offset - startOffset < submitMulti.Length)
            {
